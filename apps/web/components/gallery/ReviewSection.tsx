...
const STAR = (
  <svg viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5 text-[#FBBC04]">
...
const TrustpilotIcon = () => (
  <svg viewBox="0 0 24 24" className="h-6 w-6 shrink-0" aria-hidden fill="#00B67A">
...
export function ReviewSection({
  ...
  return (
    <div className="overflow-hidden rounded-card border border-border bg-surface shadow-card">
      {/* Header */}
      <div className="flex items-start gap-3 border-b border-border bg-[#FFFBF0] px-4 py-3.5">
        <span className="mt-0.5 text-xl leading-none">⭐</span>
        <div>
          <p className="text-sm font-bold text-ink">Tu as aimé l&apos;expérience ?</p>
          <p className="mt-0.5 text-xs text-ink-2">
            30 secondes pour aider {operatorName} 🙏
          </p>
        </div>
      </div>

      {/* Platform rows */}
      {platforms.map(({ href, platform, label, sublabel, icon }, i) => (
        <ReviewLink
          key={platform}
          token={token}
          href={href}
          platform={platform}
          className={`flex w-full items-center gap-3 px-4 py-3.5 transition hover:bg-canvas active:bg-canvas ${
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5">
              {STARS}
              <span className="text-[11px] text-ink-2">{sublabel}</span>
            </div>
          </div>
          <ChevronRight />
        </ReviewLink>
