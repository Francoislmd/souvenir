...
*Dernière mise à jour : 11 juin 2026. Si tu prends une décision d'architecture non couverte ici, documente-la en bas de ce fichier dans une section `## Décisions prises en cours de route` avant d'implémenter.*
...
- **Refonte de la galerie client en feed type Instagram + couleur de marque galerie fixe** (amende §5 Flux C.2, §8 white-label : la galerie n'est plus personnalisable par couleur opérateur) :
  ...
  - **Couleur de marque galerie fixe (`#F0386B`, rose/framboise vif)** : `apps/web/lib/brand.ts` exporte `CLIENT_BRAND_COLOR = "#F0386B"`, utilisé pour `--accent`/`--accent-tint` dans `app/g/[token]/page.tsx` et `confidentialite/page.tsx` (plus de surcharge par `operator.brandColor`), et pour le dégradé d'en-tête de l'email de livraison (`lib/email.ts`). **`Operator.brandColor` supprimé du schéma Prisma** (et de `OnboardingForm`/`SettingsForm`/API `operator`/`operator/settings`/seed/`send-delivery`/`send-test-email`) — l'opérateur ne peut plus personnaliser la couleur de sa galerie ; l'identité visuelle Souvenir (rose framboise côté client, tangerine côté dashboard pro) reste cohérente d'une école à l'autre.
